...
/* ========================================= HELPERS ========================================= */
function getProductPath(folder) { return `assets/products/${folder}`; }
function getCover(folder) { return `${getProductPath(folder)}/cover.png`; }
function getEmoji(folder) { return `${getProductPath(folder)}/emoji.png`; }
...
function saveTheme(main, sub) {
  localStorage.setItem("theme", JSON.stringify({ main, sub }));
}
...
function loadSavedTheme() {
  ...
  applyTheme(savedTheme.main, savedTheme.sub);
}
...
function addItem(product) {
  ...
  if (existing) {
    ...
    cart.push({
      ...
      coverImage: getCover(product.folder),
      emojiImage: getEmoji(product.folder),
      quantity: 1,
      stickers: [sticker]
    });
  }
...
function renderSuggestions() {
  ...
  products
    ...
    .forEach(product => {
      ...
      div.innerHTML = `
        <img src="${getCover(product.folder)}">
        <p>${product.name}</p>
      `;
      div.addEventListener("click", () => addItem(product));
...
function renderCart() {
  ...
  cart.forEach(item => {
    ...
    li.innerHTML = `
      <div class="cart-left">
        <img class="cart-img" src="${getCover(item.folder)}">
        <span>${item.name}</span>
      </div>
    ...
    `;
    li.querySelector(".minus-btn").addEventListener("click", () => removeItem(item.id));
  ...
  if (orderData) { orderData.value = cart.map(item => `${item.name} x${item.quantity}`).join(" | "); }
  if (totalData) { totalData.value = total.toLocaleString() + " VND"; }
}

/* ========================================= VARIANT PANEL ========================================= */
function openVariantPanel(sticker, item, element) {
  if (!variantPanel) return;
  variantPanel.innerHTML = "";
  
  const rect = element.getBoundingClientRect();
  variantPanel.style.display = "flex";
  variantPanel.style.left = `${rect.left + rect.width / 2}px`;
  variantPanel.style.top = `${rect.bottom + 16}px`;
  variantPanel.style.transform = "translateX(-50%)";
  
  /* ========================= COLOR ========================= */
  const mainLabel = document.createElement("div");
  mainLabel.className = "variant-label";
  mainLabel.innerText = "Color";
  variantPanel.appendChild(mainLabel);
  
  const mainRow = document.createElement("div");
  mainRow.className = "variant-row";
  for (let i = 1; i <= 20; i++) {
    const img = document.createElement("img");
    img.src = `assets/products/${item.folder}/icon/${i}.png`;
  ...
  subLabel.className = "variant-label";
  subLabel.innerText = "Other";
  variantPanel.appendChild(subLabel);
  
  const subRow = document.createElement("div");
  subRow.className = "variant-row";
  for (let i = 1; i <= 20; i++) {
    const img = document.createElement("img");
    img.src = `assets/products/${item.folder}/icon/sub/${i}.png`;
    img.onerror = () => { img.remove(); };
    img.onclick = () => {
...
function renderBox() {
  ...
  cart.forEach(item => {
    item.stickers.forEach(sticker => {
      ...
      wrap.style.left = sticker.x + "px";
      wrap.style.top = sticker.y + "px";
      wrap.addEventListener("dblclick", (e) => {
        e.stopPropagation();
        openVariantPanel(sticker, item, wrap);
      });
      
      const mainImg = document.createElement("img");
      mainImg.className = "sticker-main";
      let imagePath = sticker.image || item.emojiImage;
      if (sticker.mainVariant) {
        imagePath = `assets/products/${item.folder}/icon/${sticker.mainVariant}.png`;
      }
      mainImg.src = imagePath;
      wrap.appendChild(mainImg);
      
      if (sticker.subVariant) {
        const subImg = document.createElement("img");
        subImg.className = "sticker-sub";
        subImg.src = `assets/products/${item.folder}/icon/sub/${sticker.subVariant}.png`;
        wrap.appendChild(subImg);
      }
      
      enableDragging(wrap, sticker, item);
...
function enableDragging(el, sticker, item) {
  ...
  el.addEventListener("pointermove", (e) => {
    ...
    // Freely update dragging coordinates across the viewport
  ...
  function stopDragging() {
    ...
    if (touchingBin) {
      ...
      renderSuggestions();
      renderBox();
      if (bin) bin.classList.remove("bin-active");
      return;
    }
    ...
    // Restrict coordinates inside canvas boundaries
...
document.addEventListener("click", (e) => {
  if (variantPanel && !e.target.closest("#variantPanel") && !e.target.closest(".sticker")) {
    variantPanel.style.display = "none";
  }
});
